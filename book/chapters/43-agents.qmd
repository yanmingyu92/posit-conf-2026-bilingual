---
title: "4.3 智能体 · Agents & Tool Calling"
objectives: |
...
- 能用 ellmer 建会话、设 system prompt、检查消息列表；见过 `type_*()` 描述符（→ 不熟？先回 [4.1]、[4.2] §2）

::: {.callout-note}
本章情境沿用 llms 工作坊的「最后一家 Blockbuster 录像带店」：
`blockbuster/` 文件夹里有 `members.csv`、`rentals.csv`、`dues.csv` 和
店长留言 `notes.md`——这是 agent 的全部世界。
:::
...
play_sound <- function(sound) {
  beepr::beep(sound)                            # 副作用真实发生在 R 这边
...
数据助理第一版只要两个工具，但三个设计决定要先记下：① agent 的世界被 `proj_path()`**锁死在 `blockbuster/` 内**（沙箱）；② `write_file` 返回回执；③ system prompt 规定「需要代码时，**写脚本给人跑**」——agent 不执行代码，人是执行环，这是最朴素也最有效的安全阀。
...
edit_file <- function(path, old, new) {
  ...
  if (identical(matches, -1L)) cli::cli_abort("Could not find the text to replace in {path}.")
...
1. **沙箱**：`proj_path()` 锁根目录；外部文件名再套 `basename()` 白名单（见 [4.6] §4）
