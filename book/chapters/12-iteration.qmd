...
能独立完成以下两问再继续；否则先回补 1.1 Writing Functions（[tidyverse 入门要求](../index.qmd#prerequisites)）：
...
::: {.callout-warning}
## 高频错误：两个输入长度不齐
`map2()` 要求输入等长，或其中一个输入长度为 1（此时会重循环）。
长度 2 与 3 等不兼容组合会**直接报错**，不会静默按较长输入重循环。
若业务要求严格一一配对，先 `stopifnot(length(a) == length(b))`。
:::
