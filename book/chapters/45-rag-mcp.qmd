...
# 4.5 RAG 与 MCP · Retrieval and Tool Protocols
...
- 会用 ellmer 建会话、设 system prompt（→ 不熟？先回 [4.1](41-llm-basics.qmd)）
- 知道工具注册与 agent 循环（→ [4.3](43-agents.qmd)；§5 的 `ragnar_register_tool_retrieve()`
  本质就是一个预制检索工具）
...
工具」的接线标准化。心智模型：**USB 接口**——客户端（Posit Assistant、
...
4. 打开 Posit Assistant，选中一段 polars 代码，提示词末尾加 `#context7`：
...
1. **检索会漏**：top-k 近邻不保证语义等价；关键问答必须回到
   「问题集 → 命中检查」的评测循环（→ [4.2](42-structured-output.qmd)）
...
交付对照表，并回答：哪种设置更优，代价是什么？（评测纪律参照 [4.2](42-structured-output.qmd)）
:::
