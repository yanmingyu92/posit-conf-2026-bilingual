...
- 跑通过 4.1 的 `extract_data()`、见过 `type_object()` / `type_array()`，并记得铁律：**模型负责格式，事实必须有来源**（→ 不熟？先回 [4.1] §5）

::: {.callout-note}
命名提示：ellmer 新版把 4.1 的 `extract_data()` 更名为 `chat_structured()`、
`set_system()` 更名为 `set_system_prompt()`——同一契约，本章统一用新名。
:::
...
cases <- tibble::tibble(
  id = c("sensor", "units", "plants"),
  input = c(sensor_prompt, units_prompt, plants_prompt),
  target = c(sensor_criteria, units_criteria, plants_criteria)
)  # 每条 input 埋一个数据质量瑕疵，target 写明「发现它」的标准（llms `15_evals`）

task <- Task$new(
  dataset = cases,
  solver = generate(chat_posit(model = "claude-haiku-4-5")),
  scorer = model_graded_qa(scorer_chat = chat_posit(model = "claude-sonnet-5"))
)

task$eval(epochs = 2)  # 跑 2 遍看方差；vitals_view() 可逐例查看判分
task$get_samples() |>
