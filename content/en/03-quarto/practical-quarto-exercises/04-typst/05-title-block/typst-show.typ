...
$if(logo)$
  ...
  logo-alt: "$logo.alt$",
$endif$
$if(brs-release.period)$
  release-period: [$brs-release.period$],
$endif$
$if(brs-release.region)$
  release-region: [$brs-release.region$],
$endif$
$if(brs-release.status)$
  status: [$brs-release.status$],
$endif$
$-- brand.color.* is not exposed to Pandoc, but brand-color is a Typst
$-- variable defined above this partial, so pass it straight through.
  accent: brand-color.primary,
  muted: brand-color.secondary,
$if(by-author)$
  authors: (
$for(by-author)$
$if(it.name.literal)$
