...
```
Pass the labeling function to `scale_*_continuous(labels = ...)`. Also, `scales::label_percent()` makes sense only for **proportions**. The histogram's x-axis contains raw millimeters; forcing percentage labels onto it creates absurd results or an error. **Identify what the axis values represent before choosing a label function.**
:::
